...
## [Unreleased] - 2026-09-14 (Gauntlet A1 + F2.1 — raiz base única, caminhos finais e certificado no container)

### Alterado
- **Modelo de privilégio: o `cctl` passa a exigir `sudo`** (pode rodar como usuário normal, desde que tenha permissão de sudo) — a engenharia de "funcionar sem privilégio" fica adiada. `core_priv_run`/`core_sudo_usable` (`lib/core.sh`) não mudaram neste round (isso é o round A2).
- **Raiz base única e configurável**: `CCTL_BASE_DIR` (`cctl.conf`, default `/opt/cctl`), de onde derivam `CCTL_INSTANCE_BASE_DIR` (`${CCTL_BASE_DIR}/instances`), `NGINX_VHOSTS_DIR` e `LETSENCRYPT_DIR` (todo o estado do `nginx-proxy` mora sob `${CCTL_BASE_DIR}/nginx-proxy/` — F2.1) — cada folha mantém escape hatch próprio. `CRON_DIR`/`LOGROTATE_DIR` continuam fixos (`/etc/cron.d`, `/etc/logrotate.d` — cron/logrotate leem caminho fixo, root-owned). Corrigido de graça o bug de ordem em que `LETSENCRYPT_LIVE_DIR` derivava de `LETSENCRYPT_DIR` **antes** dele ser definido (customizar só `LETSENCRYPT_DIR` agora propaga para o `live`).
- **Certificado: um único destino, dentro do container (F2.1)** — `SSL_CERTS_DIR` e `CERTBOT_WEBROOT_DIR` foram fundidos/removidos: `LETSENCRYPT_DIR` (`${CCTL_BASE_DIR}/nginx-proxy/certs`) é agora o único ponto de certificado, montado **RW** (não mais `:ro`) em `/etc/letsencrypt` no container `nginx-proxy`. `ssl_get_cert_path`/`ssl_get_key_path` (`lib/ssl.sh`): o ramo `manual|self-signed` passa a apontar para `/etc/letsencrypt/<domínio>/...` (era `/etc/nginx/certs/<domínio>/...`), compartilhando a raiz com o ramo `letsencrypt` (`/etc/letsencrypt/live/<domínio>/...`). O webroot do certbot (`/var/www/certbot`) já existe na imagem do `nginx-proxy` — sem mount, sem variável de host.
- **certbot roda dentro do container do proxy (F2.1)** — `_ssl_issue_letsencrypt`/`ssl_renew` (`lib/ssl.sh`) trocam `sudo certbot ...` no host por `docker exec ${NGINX_CONTAINER_NAME} certbot ...`; sem as flags `--config-dir`/`--work-dir`/`--logs-dir` (dentro do container os defaults do certbot já apontam para `/etc/letsencrypt` etc). **O host deixa de precisar do binário `certbot`** — removido dos pré-requisitos (`README.md`, `docs/USAGE.md`). O job periódico de renovação (cron dentro da imagem) é responsabilidade da F3 — fora deste round.
- **Consolidação de defaults duplicados**: o literal `/opt/cctl` (e cada derivação) passa a existir em um único lugar (`cctl.conf`) — `lib/nginx.sh`, `lib/ssl.sh` e `commands/list.sh` não redeclaram mais os defaults (`commands/list.sh` também deixou de referenciar a variável legada `INSTANCE_BASE_DIR`, sem consumidor conhecido em código/templates/docs).
- **`nginx_proxy_up`** (`lib/nginx.sh`) passa a criar a árvore inteira a partir de `CCTL_BASE_DIR` (inclusive `instances/`) e, na sequência, ajusta o dono das folhas operacionais (`CCTL_INSTANCE_BASE_DIR`, `NGINX_VHOSTS_DIR`) para o usuário que invocou (`${SUDO_USER:-$(id -un)}`, via `chown` direto — fora do contrato de `core_priv_run`) — o dia a dia (`install`/`up`/`backup`/`rollout`) deixa de pedir sudo depois do primeiro `cctl proxy up`. `LETSENCRYPT_DIR` fica de fora do chown de propósito (continua root-owned — guarda chave privada, e é o certbot, como root dentro do container, quem escreve ali).
- **Novo comando `cctl paths`** (`commands/paths.sh`): mostra `CCTL_BASE_DIR` em uso, cada raiz efetiva (derivada e fixa) e se existe/é gravável — liberado em qualquer contexto (`init|help|proxy|paths`, `lib/core.sh`).
- Qualidade: 381 → 400 testes Bats (0 falhas) — nova bateria `tests/paths.bats` (derivação de `cctl.conf` em subshell isolado + `cctl paths`); `tests/proxy.bats`/`tests/ssl.bats` reescritos onde afirmavam os caminhos/montagens antigos, com provas dedicadas de que o mount de `/etc/letsencrypt` não regride para `:ro` e de que o webroot/segundo destino de certificado não voltam a ser montados; `tests/install.bats` ajustado para os novos caminhos de `SSL_CERT_PATH`/`SSL_KEY_PATH` — `shellcheck -S warning` limpo.
- **Fora deste round (A2, próximo)**: simplificação de `core_priv_run`, remoção do fallback de crontab de usuário (`lib/cron.sh`) e do aviso "warn-only" do logrotate no template Moodle.
...
- **O switch do Blue/Green reescreve o vhost VIVO em `NGINX_VHOSTS_DIR` (`${CCTL_BASE_DIR}/nginx/vhosts.d/<projeto>.conf`, default `/opt/cctl/nginx/vhosts.d/<projeto>.conf` a partir do round "raiz base unica" — ver entrada correspondente abaixo), nao o `./nginx/site.conf` da instancia** — este ultimo continua sendo o render base do `cctl install` e nao reflete o slot ativo apos um rollout. `cctl rollout status` (ou o proprio vhost vivo) e a fonte da verdade sobre o slot em trafego, nunca o `site.conf` da instancia.
